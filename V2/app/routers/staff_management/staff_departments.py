...
def create_staff_department(data:StaffDepartmentCreate, db: Session = Depends(get_db)):
...
def get_staff_departments(filters: DepartmentFilterParams = Depends(), db: Session = Depends(get_db)):
        staff_departments_crud = StaffDepartmentCrud(db)
