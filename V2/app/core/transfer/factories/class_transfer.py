...
class ClassTransferFactory:
    """Factory class for managing class transfer operations."""

    def __init__(self, session: Session, model=ClassTransfer):
        self.model = model
    ...
    def create_transfer(self, student_id: UUID, data) -> ClassTransfer:
        new_transfer = ClassTransfer(
            id=uuid4(),
    ...
    def get_transfer(self, transfer_id: UUID) -> ClassTransfer:
        try:
            return self.repository.get_by_id(transfer_id)
        except EntityNotFoundError as e:
            self.raise_not_found(transfer_id, e)

    def get_all_transfers(self, filters) -> List[ClassTransfer]:
        return self.repository.execute_query(['academic_session', 'status'], filters)

    @resolve_fk_on_update()
    def update_transfer(self, transfer_id: UUID, data: dict) -> ClassTransfer:
    ...
    def archive_transfer(self, transfer_id: UUID, reason) -> ClassTransfer:
        try:
    ...
    def get_all_archived_transfers(self, filters) -> List[ClassTransfer]:
        return self.repository.execute_archive_query(['academic_session', 'status'], filters)

    def get_archived_transfer(self, transfer_id: UUID) -> ClassTransfer:
        try:
            return self.repository.get_archive_by_id(transfer_id)
        except EntityNotFoundError as e:
            self.raise_not_found(transfer_id, e)

    def restore_transfer(self, transfer_id: UUID) -> ClassTransfer:
        try:
