...
class Subjects(Base, AuditMixins, TimeStampMixins, ArchiveMixins):
    ...
    group: Mapped[SubjectGroup] = mapped_column(Enum(SubjectGroup, name = 'subjectgroup'), nullable = True)
    is_elective: Mapped[bool] = mapped_column(default = True)
    educator_id: Mapped[UUID] = mapped_column(ForeignKey('educator.id', ondelete='SET NULL'), nullable=True)
