...
def test_model_structure_column_data_types(db_inspector):
    ...
    enum_checks = {"code": ClassCode, "archive_reason": ArchiveReason}
...
def test_model_structure_unique_constraints(db_inspector):
    ...
    assert has_staff_dept_constraint, (
        "classes should have a unique constraint on " "level and code"
    )
...
def test_model_structure_foreign_keys(db_inspector):
    ...
    mentor_fk = next(
        (fk for fk in foreign_keys if fk["constrained_columns"] == ["mentor_id"]), None
    )
    student_rep_fk = next(
    ...
    assistant_rep_fk = next(
        (
            ...
            if fk["constrained_columns"] == ["assistant_rep_id"]
        ),
        None,
    )
