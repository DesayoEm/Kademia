...
def test_model_structure_foreign_keys(db_inspector):
    ...
    previous_level_fk = next(
        (
            ...
            if fk["constrained_columns"] == ["previous_level_id"]
        ),
        None,
    )
    new_level_fk = next(
    ...
    previous_department_id = next(
        (
            fk
            for fk in foreign_keys
            if fk["constrained_columns"] == ["previous_department_id"]
        ),
        None,
    )
    new_department_id = next(
        (
            fk
            for fk in foreign_keys
            if fk["constrained_columns"] == ["new_department_id"]
        ),
        None,
    )
    ...
    ), "Missing foreign key for previous_department_id"
    assert (
        previous_department_id["options"]["ondelete"].upper() == "RESTRICT"
    ...
    assert (
        new_department_id["options"]["ondelete"].upper() == "RESTRICT"
