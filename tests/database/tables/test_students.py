...
def test_model_structure_unique_constraints(db_inspector):
    ...
    assert any(
        columns == ["student_id"] for columns in constraints_map.values()
...
def test_model_structure_foreign_keys(db_inspector):
    ...
    class_fk = next(
        (fk for fk in foreign_keys if fk["constrained_columns"] == ["class_id"]), None
    )
    level_fk = next(
    ...
    assert (
        class_fk["options"]["ondelete"].upper() == "RESTRICT"
    ...
    assert (
        guardian_fk["options"]["ondelete"].upper() == "RESTRICT"
