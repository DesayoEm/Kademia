...
def test_model_structure_column_data_types(db_inspector):
    ...
    enum_checks = {"previous_level": AccessLevel, "new_level": AccessLevel}
...
def test_model_structure_foreign_keys(db_inspector):
    ...
    staff_fk = next(
        (fk for fk in foreign_keys if fk["constrained_columns"] == ["staff_id"]), None
    )
    assert staff_fk is not None, "Missing foreign key for staff_id"
    assert (
        staff_fk["options"]["ondelete"].upper() == "CASCADE"
    ), "level_id should CASCADE on delete"

    changed_by_fk = next(
        (fk for fk in foreign_keys if fk["constrained_columns"] == ["changed_by_id"]),
        None,
    )
    assert changed_by_fk is not None, "Missing foreign key for changed_by_id"
    assert (
        changed_by_fk["options"]["ondelete"].upper() == "RESTRICT"
