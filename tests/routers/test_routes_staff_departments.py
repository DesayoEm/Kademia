...
def mock_crud():
    ...
    valid_response = StaffDepartmentResponse(
        name="Test Department", description="Test Description", manager_id=None
    )
    response_dict = valid_response.model_dump()

    with patch(
        "V2.app.academic_structure.staff_management.staff_departments.StaffDepartmentCrud"
    ) as mock:
        ...
        mock_instance.update_department.return_value = {
            **response_dict,
...
def test_create_department(mock_crud):
    ...
    response = client.post(
        "/api/v1/staff/departments", json=department_data.model_dump()
    )
...
def test_update_department(mock_crud):
    ...
    update_data = StaffDepartmentUpdate(
        name="Updated Department", description="Human Resources"
    )

    response = client.put(
        f"/api/v1/staff/departments/{test_uuid}", json=update_data.model_dump()
    )
    print("Mock Calls:", mock_instance.mock_calls)
