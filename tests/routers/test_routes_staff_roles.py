...
def mock_crud():
    ...
    with patch(
        "V2.app.academic_structure.staff_management.staff_roles.StaffRoleCrud"
    ) as mock:
        ...
        mock_instance.update_role.return_value = {
            **response_dict,
            "name": "Updated role",
        }
...
def test_create_role(mock_crud):
    ...
    role_data = StaffRoleCreate(name="HR role", description="Human Resources")
...
def test_update_role(mock_crud):
    ...
    mock_instance, test_uuid = mock_crud
    update_data = StaffRoleUpdate(name="Updated role", description="Human Resources")

    response = client.put(
        f"/api/v1/staff/roles/{test_uuid}", json=update_data.model_dump()
    )
    print("Mock Calls:", mock_instance.mock_calls)
