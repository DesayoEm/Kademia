...
# Active routers
@router.post(
    "/{student_id}/assessments/{grade_id}/file",
...
def upload_assessment_file(
    ...
    service: AssessmentFileService = Depends(
        get_authenticated_service(AssessmentFileService)
    ),
    grade_factory: GradeFactory = Depends(get_authenticated_factory(GradeFactory)),
    student_factory: StudentFactory = Depends(
        get_authenticated_factory(StudentFactory)
    ),
):
...
@router.post(
    "/grades/{student_subject_id}", response_model=GradeResponse, status_code=201
)
def grade_student(
    student_id: UUID,
...
@router.get(
    "/grades/{grade_id}/audit/export", response_class=FileResponse, status_code=200
)
