...
def get_archived_qualifications(
    ...
    factory: QualificationFactory = Depends(
        get_authenticated_factory(QualificationFactory)
    ),
):
...
@router.get(
    "/archived/qualifications/{qualification_id}", response_model=QualificationResponse
)
...
@router.post(
    "/qualifications/{qualification_id}", response_class=FileResponse, status_code=204
)
def export_qualification(
    qualification_id: UUID,
    export_format: ExportFormat,
    service: StaffManagementService = Depends(
        get_authenticated_service(StaffManagementService)
    ),
):
    file_path = service.export_qualification_audit(
        qualification_id, export_format.value
    )
