...
from app.core.curriculum.factories.academic_level_subject import (
...
from app.core.curriculum.schemas.academic_level_subject import (
    AcademicLevelSubjectCreate,
    AcademicLevelSubjectFilterParams,
...
# Archive routers
@router.get(
    "/archive/level-subjects/", response_model=List[AcademicLevelSubjectResponse]
)
def get_archived_level_subjects(
    filters: AcademicLevelSubjectFilterParams = Depends(),
    factory: AcademicLevelSubjectFactory = Depends(
        get_authenticated_factory(AcademicLevelSubjectFactory)
    ),
):
...
@router.get(
    "/archive/level-subjects/{level_subject_id}",
    response_model=AcademicLevelSubjectResponse,
)
...
@router.get(
    "/level-subjects/{level_subject_id}/educators",
...
@router.get(
    "/level-subjects/{level_subject_id}/audit", response_model=AcademicLevelSubjectAudit
)
...
@router.get(
    "/level-subjects/{level_subject_id}", response_model=AcademicLevelSubjectResponse
)
...
@router.post(
    "/level-subjects/{level_subject_id}", response_class=FileResponse, status_code=204
)
def export_level_subject_audit(
    ...
    file_path = service.export_level_subject_audit(
        level_subject_id, export_format.value
    )
