...
class ArchiveDependencyError(ArchiveAndDeleteError):
    ...
    def __init__(
        self, entity_model, identifier: UUID, display_name: str, related_entities: str
    ):
        super().__init__()
        self.user_message = (
            f"Cannot archive {display_name} while linked to active {related_entities}."
        )
...
class DeletionDependencyError(ArchiveAndDeleteError):
    """Raised when attempting to delete an entity that is referenced by active records"""
    ...
    def __init__(
        ...
        super().__init__()
        self.user_message = (
            f"Cannot delete {display_name} while linked to {related_entities}."
        )
