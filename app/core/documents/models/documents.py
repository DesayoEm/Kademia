...
class StudentDocument(Base, AuditMixins, TimeStampMixins, ArchiveMixins):
    ...
    student_id: Mapped[UUID] = mapped_column(ForeignKey('students.id',
            ondelete='CASCADE',name='fk_student_documents_students_student_id'),default=uuid4
        )
    ...
    # Relationships
    document_owner: Mapped['Student'] = relationship(back_populates='documents_owned', foreign_keys='[StudentDocument.student_id]')
...
class StudentAward(Base, AuditMixins, TimeStampMixins, ArchiveMixins):
    ...
    # Relationships
    award_owner: Mapped['Student'] = relationship(back_populates='awards_earned',
