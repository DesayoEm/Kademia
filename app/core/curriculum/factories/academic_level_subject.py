...
class AcademicLevelSubjectFactory(BaseFactory):
    ...
    @resolve_unique_violation(
        {
            "academic_level_subjects_level_id_subject_id_key": (
                "_",
                "This subject is already assigned to this level",
            ),
            "academic_level_subjects_code_key": (
                "name",
                lambda self, _, data: data.code,
            ),
        }
    )
    @resolve_fk_on_create()
    def create_academic_level_subject(
        self, level_id: UUID, data
    ...
    def get_academic_level_subject(
        self, academic_level_subject_id: UUID
    ) -> AcademicLevelSubject:
    ...
    def archive_academic_level_subject(
        self, academic_level_subject_id: UUID, reason
    ) -> AcademicLevelSubject:
        ...
        try:
            failed_dependencies = self.archive_service.check_active_dependencies_exists(
                entity_model=self.model, target_id=academic_level_subject_id
            )
            if failed_dependencies:
                raise ArchiveDependencyError(
                    entity_model=self.entity_model,
                    identifier=academic_level_subject_id,
                    display_name=self.display_name,
                    related_entities=", ".join(failed_dependencies),
                )
            return self.repository.archive(
                academic_level_subject_id, self.actor_id, reason
            )
    ...
    def delete_academic_level_subject(self, academic_level_subject_id: UUID) -> None:
        ...
        try:
            failed_dependencies = self.delete_service.check_active_dependencies_exists(
                self.model, academic_level_subject_id
            )
    ...
    def get_all_archived_academic_level_subjects(
        self, filters
    ...
    def get_archived_academic_level_subject(
        self, academic_level_subject_id: UUID
    ) -> AcademicLevelSubject:
    ...
    def restore_academic_level_subject(
        self, academic_level_subject_id: UUID
    ) -> AcademicLevelSubject:
    ...
    @resolve_fk_on_delete(display="level subject")
    def delete_archived_academic_level_subject(
        self, academic_level_subject_id: UUID
    ) -> None:
