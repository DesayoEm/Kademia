...
class Subject(Base, AuditMixins, TimeStampMixins, ArchiveMixins):
    ...
    department_id: Mapped[UUID] = mapped_column(
        ForeignKey(
            ...
            ondelete="RESTRICT",
            name="fk_subjects_student_departments_department_id",
        ),
        nullable=True,
    )

    # Relationships
    academic_level_subject: Mapped[List["AcademicLevelSubject"]] = relationship(
        back_populates="base_subject"
    )
    department: Mapped["StudentDepartment"] = relationship(
        back_populates="subjects", foreign_keys="[Subject.department_id]"
    )
...
class AcademicLevelSubject(Base, AuditMixins, TimeStampMixins, ArchiveMixins):
    ...
    level_id: Mapped[UUID] = mapped_column(
        ForeignKey(
            "academic_levels.id",
            ondelete="RESTRICT",
            name="fk_academic_level_subjects_academic_levels_level_id",
        )
    )
    subject_id: Mapped[UUID] = mapped_column(
        ForeignKey(
            "subjects.id",
            ondelete="RESTRICT",
            name="fk_academic_level_subjects_subjects_subject_id",
        )
    )
    ...
    # Relationships
    base_subject: Mapped["Subject"] = relationship(
        back_populates="academic_level_subject",
        foreign_keys="[AcademicLevelSubject.subject_id]",
    )
    level: Mapped["AcademicLevel"] = relationship(
        back_populates="subjects", foreign_keys="[AcademicLevelSubject.level_id]"
    )
...
class StudentSubject(Base, AuditMixins, TimeStampMixins, ArchiveMixins):
    ...
    # Relationships
    subject: Mapped[List["AcademicLevelSubject"]] = relationship(
...
class SubjectEducator(Base, AuditMixins, TimeStampMixins, ArchiveMixins):
    ...
    educator_id: Mapped[UUID] = mapped_column(
        ForeignKey(
            "educators.id",
            ondelete="RESTRICT",
            name="fk_subject_educators_educators_educator_id",
        )
    )
    ...
    subject: Mapped[List["AcademicLevelSubject"]] = relationship(
        ...
        foreign_keys="[SubjectEducator.academic_level_subject_id]",
    )
    teacher: Mapped["Educator"] = relationship(
        back_populates="subject_assignments",
        foreign_keys="[SubjectEducator.educator_id]",
    )

    __table_args__ = (
        Index(
            "idx_subject_level_educator",
            "educator_id",
            "academic_level_subject_id",
            "academic_session",
        ),
