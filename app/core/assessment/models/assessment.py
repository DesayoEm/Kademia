...
class Grade(Base, AuditMixins, TimeStampMixins, ArchiveMixins):
    ...
    student_id: Mapped[UUID] = mapped_column(
        ForeignKey(
            "students.id", ondelete="CASCADE", name="fk_grades_students_student_id"
        )
    ...
    graded_by: Mapped[UUID] = mapped_column(
        ForeignKey("staff.id", ondelete="RESTRICT", name="fk_grades_educator_graded_by")
    )
    graded_on: Mapped[pydate] = mapped_column(Date)

    # Relationships
    student: Mapped["Student"] = relationship(
        back_populates="grades", foreign_keys="[Grade.student_id]", passive_deletes=True
    )
    grader: Mapped["Educator"] = relationship(foreign_keys="[Grade.graded_by]")
    student_subject: Mapped["StudentSubject"] = relationship(
        back_populates="grades", foreign_keys="[Grade.student_subject_id]"
    )
...
class TotalGrade(Base, AuditMixins, TimeStampMixins, ArchiveMixins):
    ...
    # Relationships
    student_subject: Mapped["StudentSubject"] = relationship(
        back_populates="total_grade", foreign_keys="[TotalGrade.student_subject_id]"
    )
    student: Mapped["Student"] = relationship(
        back_populates="total_grades", foreign_keys="[TotalGrade.student_id]"
    )

    __table_args__ = (Index("idx_total_grade_score", "total_score"),)
