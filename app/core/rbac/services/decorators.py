...
def require_permission(resource: Resource, action: Action,resource_id:UUID | None = None):
    """
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            handler =  (
                kwargs.get('factory') or
                kwargs.get('service')
            )
            if not handler:
                raise PermissionHandlerError("No factory or service found in route parameters")

            if not hasattr(handler, 'session') or not hasattr(handler, 'current_user'):
                raise PermissionHandlerError("Handler must have session and current_user attributes")
