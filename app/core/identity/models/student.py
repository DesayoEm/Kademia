...
class Student(UserBase):
    ...
    user_type: Mapped[UserType] = mapped_column(
        Enum(UserType, name="usertype"), default=UserType.STUDENT
    )
    status: Mapped[StudentStatus] = mapped_column(
        Enum(StudentStatus, name="studentstatus"), default=StudentStatus.ENROLLED
    )
    ...
    level_id: Mapped[UUID] = mapped_column(
        ...
        ForeignKey(
            "academic_levels.id",
            ondelete="SET NULL",
            name="fk_students_academic_levels_level_id",
        ),
        nullable=True,
    )
    class_id: Mapped[UUID] = mapped_column(
        UUID,
        ForeignKey(
            "classes.id", ondelete="SET NULL", name="fk_students_classes_class_id"
        ),
        nullable=True,
    )
    department_id: Mapped[UUID] = mapped_column(
        ...
        ForeignKey(
            ...
            ondelete="SET NULL",
            name="fk_students_student_departments_department_id",
        ),
        nullable=True,
    )
    ...
    # Relationships
    documents_owned: Mapped[List["StudentDocument"]] = relationship(
        back_populates="document_owner"
    )
    awards_earned: Mapped[List["StudentAward"]] = relationship(
        back_populates="award_owner"
    )
    guardian: Mapped["Guardian"] = relationship(
        back_populates="wards", foreign_keys="[Student.guardian_id]"
    )
    class_: Mapped["Classes"] = relationship(
        back_populates="students",
        foreign_keys="[Student.class_id]",
        primaryjoin="Student.class_id == Classes.id",
    )
    department: Mapped["StudentDepartment"] = relationship(
        back_populates="students", foreign_keys="[Student.department_id]"
    )
    level: Mapped["AcademicLevel"] = relationship(
        back_populates="students", foreign_keys="[Student.level_id]"
    )
    subjects_taken: Mapped[List["StudentSubject"]] = relationship(
        back_populates="student"
    )
    grades: Mapped[List["Grade"]] = relationship(back_populates="student")
    total_grades: Mapped["TotalGrade"] = relationship(back_populates="student")
    classes_repeated: Mapped[List["Repetition"]] = relationship(
        back_populates="repeating_student"
    )
    promotions: Mapped[List["Promotion"]] = relationship(
        back_populates="promoted_student"
    )
    department_transfers: Mapped[List["DepartmentTransfer"]] = relationship(
        back_populates="transferred_student"
    )

    represented_department: Mapped["StudentDepartment"] = relationship(
        "StudentDepartment",
        back_populates="student_rep",
