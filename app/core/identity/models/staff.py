...
class Staff(UserBase):
    ...
    user_type: Mapped[UserType] = mapped_column(
        Enum(UserType, name="usertype"), default=UserType.STAFF
    )
    staff_type: Mapped[StaffType] = mapped_column(Enum(StaffType, name="stafftype"))

    status: Mapped[StaffStatus] = mapped_column(
        Enum(StaffStatus, name="employmentstatus"), default=StaffStatus.ACTIVE
    )
    availability: Mapped[StaffAvailability] = mapped_column(
        Enum(StaffAvailability, name="staffavailability"),
    ...
    job_title_id: Mapped[Optional[UUID]] = mapped_column(
        ForeignKey(
            "staff_job_titles.id",
            ondelete="SET NULL",
            name="fk_staff_staff_job_titles_title_id",
        ),
        nullable=True,
    )
    ...
    # Relationships
    department: Mapped["StaffDepartment"] = relationship(
        back_populates="staff_members", foreign_keys="[Staff.department_id]"
    )
    title: Mapped["StaffJobTitle"] = relationship(
        back_populates="staff_members", foreign_keys="[Staff.job_title_id]"
    )
    role: Mapped["Role"] = relationship(
        back_populates="staff_members", foreign_keys="[Staff.current_role_id]"
    )
    ...
    __mapper_args__ = {"polymorphic_identity": "staff", "polymorphic_on": staff_type}
...
class Educator(Staff):
    ...
    qualifications: Mapped[List["EducatorQualification"]] = relationship(
        back_populates="educator", cascade="all, delete-orphan"
    )
    subject_assignments: Mapped[List["SubjectEducator"]] = relationship(
        back_populates="teacher"
    )
    mentored_department: Mapped[List["StudentDepartment"]] = relationship(
